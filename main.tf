...
resource "aws_lb_target_group" "main"{
    name = "${local.common_name_suffix}-${var.component}"
    port = local.tg_port # if component = frontend then assign it to port 80 else port 8080
    protocol = "HTTP"
    vpc_id = local.vpc_id
...
resource "aws_autoscaling_group" "main" {
    ...
    dynamic "tag"{ # we will get the iterator with name tag
    for_each = merge(
        local.common_tags,
        {
            Name = "${local.common_name_suffix}-${var.environment}"
...
resource "aws_autoscaling_policy" "main"{
    autoscaling_group_name = aws_autoscaling_group.main.arn
    name = "${local.common_name_suffix}-${var.component}"
